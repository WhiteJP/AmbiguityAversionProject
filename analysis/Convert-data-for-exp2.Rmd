...
d <- read_csv(here("experiment2", "data", "parsedrawresultsexp2.csv"), col_types = "ffffffiiiiiififc")
d <- d %>%
  dplyr::select(-1)
...
d <- d %>%
  dplyr::select(-condition) %>% #remove old condition variables
  mutate(
    
    #create new variable for vignette type + number
    vignette = paste0(vignetteType, vignetteNumber) %>% factor(), 
    
    # all vignettes except G2-G4 (in the original currency) were coded 1-5, rather than 0-4
    vignetteConfidence = case_when(
      !vignette %in% c("G2", "G3", "G4") ~ vignetteConfidence - 1L,
      TRUE ~ vignetteConfidence
    ),
...
d$condition <- paste0(d$vignetteType, d$vignetteNumber, d$Start100) %>% factor()


#reorder data frame
d <- d[, c(1, 16, 2:4, 15, 5:14)]

glimpse(d)


#save datset to put up on osf
write_csv(d, "exp2data.csv")
