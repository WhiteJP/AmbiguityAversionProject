...
#models for AA with mean percent difference
mods.diff.pr[[1]] <- lm(meanAA ~ 1, data = d.pr)
mods.diff.pr[[2]] <- lm(meanAA ~ vignetteType + 1, data = d.pr)
mods.diff.pr[[3]] <- lm(meanAA ~ meanOddsFav + vignetteType + 1, data = d.pr)
mods.diff.pr[[4]] <- lm(meanAA ~ meanOddsFav*vignetteType + 1, data = d.pr)

## compare model fit with AIC/BIC and AIC/BIC weights. 
AICs.diff.pr <- map_dbl(mods.diff, AIC)
BICs.diff.pr <- map_dbl(mods.diff, BIC)
AICw.diff.pr <- akaike.weights(AICs.diff)
BICw.diff.pr <- akaike.weights(BICs.diff)
...
# check assumptions of models -- no violations. 
map(mods.diff.pr, function(x){plot(fitdist(x$residuals, distr = "norm"))})
...
#compare combined and original model
AICs.pr <- c(AIC(mod.combined.pr), AIC(mods.diff.pr[[2]]), AIC(mods.conf.pr[[2]]))
...
##  percent difference model preferred
mods.diff.pr[[3]] %>% lm.beta() %>% summary()

## draw graph
d.pr %>% ggplot(aes(x = meanpercdiff, y = meanAA, col = vignetteType)) + 
  geom_text(aes(label = vignette)) +
  geom_smooth(method = "lm", se = FALSE) +
  xlab("Mean percentage difference (fav - unfav)") +
  ylab("Mean ambiguity aversion rating")
...
``` {r bayes-mod-odds}
bayesmods.diff.pr <- list()

bayesmods.diff.pr[[1]] <- brm(meanAA ~ 1, data = d.pr, refresh = 0, open_progress = FALSE)
bayesmods.diff.pr[[2]] <- brm(meanAA ~ vignetteType + 1, data = d.pr, refresh = 0, open_progress = FALSE)
bayesmods.diff.pr[[3]] <- brm(meanAA ~ meanOddsFav + vignetteType + 1, data = d.pr, refresh = 0, open_progress = FALSE)
bayesmods.diff.pr[[4]] <- brm(meanAA ~ meanOddsFav*vignetteType + 1, data = d.pr, refresh = 0, open_progress = FALSE)
...
#compare diff models
bayesmods.diff.pr <- map(bayesmods.diff.pr, add_waic) # Add WAIC to brms object
bayes.diff.comparison.pr <- loo_compare(bayesmods.diff.pr[[1]], bayesmods.diff.pr[[2]], 
                                    bayesmods.diff.pr[[3]], bayesmods.diff.pr[[4]], criterion = "waic") # compare models
bayes.diff.comparison.pr 
...
loo_compare(bayesmod.combined.pr, bayesmods.diff.pr[[3]], criterion = "waic")

## combined model not preffered. 
## model with gain/loss and perc diff the best

summary(bayesmods.diff.pr[[3]])
plot(bayesmods.diff.pr[[3]])
