...
# get subject mapping
subj_map <- read_csv("subjectID-mapping.csv", na = c("", "NA", "na"))

#Get timepoint1 data
da <- read_csv("exp3a-data.csv", na = c("", "NA", "na"))
...
```{r}
demo_d %>% 
  #exclude  participants from demo analysis that somehow responded with text not numbers
...
## Priors and Ambiguity Aversion -- alltogether
cors_together <- dc_both %>% 
  summarise(
  ...
  mutate(
    ...
    label = case_when(
      p < .01 ~ paste0("paste(", label, ", '***')"),
      p < .01 ~ paste0("paste(", label, ", '**')"),
      p < .05 ~ paste0("paste(", label, ", '*')"),
      #p < .10 ~ paste0("paste(", label, ", '*')"),
      TRUE ~ label)
  )


together_plot <- dc_both %>% 
  ggplot(aes(x = prior, y = AA)) +
  geom_jitter(alpha = 0.05, height = 0.30) +
  geom_smooth(method = "lm", se = FALSE) + 
  geom_text(aes(label = label), x = 60, y = 3, col = "black", data = cors_together, fontface = "bold", parse = TRUE) +
  scale_y_continuous(breaks = -3:3, minor_breaks = NULL, limits = (c(-3.30,3.30)),
                     labels = rev(c("Strongly prefer risk (3)", "Moderately prefer risk (2)", "Slightly prefer risk (1)",
                                "Indifferent (0)", "Slightly prefer ambiguity (-1)", "Moderately prefer ambiguity (-2)", "Strongly prefer ambiguity (-3)"))) +
  labs(
    #title = "Regressing ambiguity aversion onto priors collapsing across vignettes"
    x = "Priors, P(Favourable) - P(Unfavourable)",
    y = "Ambiguity Aversion"
  ) +
...
#label = if_else(p < .05, paste0("list(", label, ", *)"), label)

# draw plot
vignette_plot <- dc_both %>% 
  ggplot(aes(x = prior, y = AA, col = direction)) +
  geom_jitter(alpha = 0.05, height = 0.30, width = 0.10) +
  geom_smooth(method = "lm", se = FALSE) +
  scale_colour_manual(values = c("dodgerblue", "red2")) +
  facet_wrap(~vignette) +
  theme(legend.position = "none") +
  geom_text(aes(label = label), x = 0, y = 2.9, col = "black", data = cors, fontface = "bold", parse = TRUE, size = 3) +
  scale_y_continuous(breaks = -3:3, minor_breaks = NULL, limits = (c(-3.30,3.30))) +
...
### plot together in 1 panels
cowplot::plot_grid(together_plot, vignette_plot,labels = "AUTO", nrow = 1)
ggsave("fig-6.png", width = 7, height = 3.5)

```

### Modeling

Bayesian and Frequentist analyses differ slightly on their preferred model. 
...
mb0 <- brm(ordered(AA) ~ 1 + (1|SubjID) + (1|vignette), data = dc_both, 
           family = cumulative(link = "logit"), cores = 4, seed = 3838)
...
tab_bayes_prior <- tibble(
  formula = map_chr(prior_bmods, function(x) as.character(x$formula)[[1]]),
  LOOIC_Estimate = map_dbl(prior_bmods, function(x) loo(x)$looic %>% round(2)),
  LOOIC_SE = map_dbl(prior_bmods, function(x) loo(x)$se_looic %>% round(2))
)
...
tab_bayes_conf <- tibble(
  formula = map_chr(conf_bmods, function(x) as.character(x$formula)[[1]]),
  LOOIC_Estimate = map_dbl(conf_bmods, function(x) loo(x)$looic) %>%  round(2),
  LOOIC_SE = map_dbl(conf_bmods, function(x) loo(x)$se_looic) %>% round(2)
)
...
tab_bayes_comb <- tibble(
  formula = map_chr(comb_bmods, function(x) as.character(x$formula)[[1]]),
  LOOIC_Estimate = map_dbl(comb_bmods, function(x) loo(x)$looic) %>%  round(2),
  LOOIC_SE = map_dbl(comb_bmods, function(x) loo(x)$se_looic) %>%  round(2)
)
...
The combined model without an interaction was the prefferred model. 
...
```{r,  class.source = "fold-show"}
#model summary
mb_combined_noint %>% summary()

#fixed effects in more detail
fixef(mb_combined_noint)

#hypothesis tests
hypothesis(mb_combined_noint, "prior < 0")
hypothesis(mb_combined_noint, "directionL < 0")
hypothesis(mb_combined_noint, "conf > 0")
```

However, because the model with the interaction effect was of similar fit (better fit in the frequentist analyses, even though it was worse in these Bayesian analyses), we also interpret the paramaters from the model with the interaction. 
...
map(prior_mods, ~ round(AIC(.x), 2)) %>%  print(digts = Inf)
...
map_dbl(conf_mods,  ~ round(AIC(.x), 2)) %>%  print(digts = Inf)
...
## ALSO GET MODEL WITH INTERACTION for paper
m_combined_int <- clmm(ordered(AA) ~ 1 + direction*prior + conf + (1|SubjID) + (1|vignette), dc_both)
m_combined_int %>% summary()
AIC(m_combined_int)
...
btw_mods[[10]] <- lm(meanAA ~ 1 + direction* meanPrior + meanConf, data = d_btw)


# show table 
tibble(
  formula = map_chr(btw_mods, function(x) as.character(x$call)[2]),
  AIC_value = map_dbl(btw_mods, ~round(AIC(.x), 2)),
  AIC_weight = qpcR::akaike.weights(AIC_value)$weights %>% round(2)
) %>% 
  flextable::flextable() %>% 
  ftExtra::span_header() %>% 
...
btw_mods_nG1[[10]] <- lm(meanAA ~ 1 + direction*meanPrior + meanConf, data = d_btw_nG1)

# show table 
tibble(
  formula = map_chr(btw_mods_nG1, function(x) as.character(x$call)[2]),
  AIC_value = map_dbl(btw_mods_nG1, ~round(AIC(.x), 2)),
  AIC_weight = qpcR::akaike.weights(AIC_value)$weights %>% round(2)
) %>% 
  flextable::flextable() %>% 
  ftExtra::span_header() %>% 
...
rel_t1 <- dc_both %>% 
  mutate(
    trialn = paste("Trial", trialn) %>% fct_relevel("Trial 10", after = Inf)
  ) %>% 
  ggplot(aes(x = prior, y = AA)) +
...
rel_t2 <- dc_both %>% 
  mutate(
    trialn_t2 = paste("Trial", trialn_t2) %>% fct_relevel("Trial 10", after = Inf)
  ) %>% 
  ggplot(aes(x = prior, y = AA)) +
  geom_jitter(alpha = 0.05) +
  geom_smooth(method = "lm", se = FALSE) +
...
cowplot::plot_grid(rel_t1, rel_t2,labels = "AUTO", nrow = 1)
ggsave("fig-7.png", width = 7, height = 3.5)
```
