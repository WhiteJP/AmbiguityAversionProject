...
knitr::opts_chunk$set(echo = TRUE, cache = TRUE)
options(digits = 3)#sets inline code, and all code to only show 3 decimal places.
...
d <- read_csv("exp1data.csv", col_types = "ffffffnnfnfc")
glimpse(d)
```
...
d$vignette <- factor(d$vignette, levels = c(sprintf("G%d", seq(1:12)), sprintf("L%d", seq(1:12))))
...
# first remove andy's testing data point
...
d_removed <- d_andyremoved[instr_fail,]
d_pass <- d_andyremoved[!instr_fail,]

#inspect responses of those we removed.
d_removed
```
...
```{r inspect-data}
# inspect data before exclusions. 
skim(d_andyremoved)

# inspect data after exclusions. 
skim(d_pass)

# how many people answered each vignette
d_pass %>% count(vignette) %>% arrange(n) %>% print(n = Inf)

# get an idea of the type of responses people gave for
d_pass$postQuestion2[!is.na(d_pass$postQuestion2)] %>% head(n = 50)
```
...
#ggsave("figures/exp1graph.png", height = 75, width = 150, units = "mm")
...
olm.bayes.nopred <-  brm(vignetteAnswer.ordfac ~ 1, data = d_pass, family = cumulative(link = "logit"), 
                         ...
                         cores = 4, seed = 1298)  #to suppress progress printing to screen for .Rmd
...
d_pass_urns <- d_pass %>% filter(vignette == "G1" | vignette == "L1")
...
olm.bayes.nopred.urns <- brm(vignetteAnswer.ordfac ~ 1, data = d_pass_urns, family = cumulative(link = "logit"), 
                         refresh = 0, open_progress = FALSE, cores = 4,  seed = 1298) 
...
#### LOOIC values
The LOOIC values for our four different Bayesian models are:
...
#### Urn hypothesis tests

```{r t-test}
## GAIN URN VIGNETTE ##

#ct test
rstatix::t_test(d_pass[d_pass$vignette == "G1", ], vignetteAnswer ~ 1, mu = 0)

# wilcox test
rstatix::wilcox_test(d_pass[d_pass$vignette == "G1", ], vignetteAnswer ~ 1, mu = 0, exact = TRUE)
rstatix::wilcox_effsize(d_pass[d_pass$vignette == "G1", ], vignetteAnswer ~ 1, mu = 0, exact = TRUE)

## LOSS URN VIGNETTE ##

#ct test
rstatix::t_test(d_pass[d_pass$vignette == "L1", ], vignetteAnswer ~ 1, mu = 0)

# wilcox test
rstatix::wilcox_test(d_pass[d_pass$vignette == "L1", ], vignetteAnswer ~ 1, mu = 0, exact = TRUE)
rstatix::wilcox_effsize(d_pass[d_pass$vignette == "L1", ], vignetteAnswer ~ 1, mu = 0, exact = TRUE)

```

### Save data for use in experiment 2
